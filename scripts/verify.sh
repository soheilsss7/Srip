...
required=(
  ...
  apps/api/prisma/schema.prisma apps/api/prisma/migrations/migration_lock.toml apps/api/prisma/migrations/20260823_phase2_infrastructure_foundation/migration.sql apps/api/prisma/migrations/20260823_phase4_authentication/migration.sql apps/api/src/app.module.ts apps/api/src/health/health.service.ts
...
test -f "$root/docs/source/سند فنی پلتفرم تعاملات چک لیستی - original.docx"
test -f "$root/docs/source/سند فنی پلتفرم تعاملات چک لیستی - LIVE BUILD 03.docx"
...
test -f "$root/apps/api/prisma/migrations/20260823_phase5_authorization_multitenancy/migration.sql" || { echo 'Phase 5 migration missing'; exit 1; }
test -f "$root/apps/api/src/common/authorization/access-policy.spec.ts" || { echo 'Phase 5 permission tests missing'; exit 1; }
python - <<PY2
from docx import Document
p="$root/docs/source/سند فنی پلتفرم تعاملات چک لیستی - LIVE BUILD 03.docx"
d=Document(p)
