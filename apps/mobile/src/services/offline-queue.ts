...
export type QueuedMutation = { id: string; path: string; method: 'POST'|'PATCH'|'DELETE'; body?: unknown; idempotencyKey?: string; createdAt: string; attempts: number };
...
// Stable idempotency key per queued mutation, reused across retries/reconnects so the
// backend replays (returns the original result) instead of creating duplicates.
