...
const TONE_FA: Record<string, string> = { POSITIVE: 'مثبت', NEGATIVE: 'منفی', NEUTRAL: 'خنثی' };

function Delta({ current, previous, invert = false }: { current: number | null; previous: number | null; invert?: boolean }) {
  if (current == null || previous == null || current === previous) return <span className="pp-muted" style={{ fontSize: 11 }}>بدون تغییر</span>;
  const up = current > previous;
  const good = invert ? !up : up;
  return (
    <span style={{ fontSize: 11, display: 'inline-flex', gap: 4, alignItems: 'center', color: good ? '#047857' : '#b91c1c' }}>
      {up ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
      {good ? 'رشد' : 'کاهش'} {fmtN(Math.abs(current - previous))}
    </span>
  );
}
...
export default function QbrPage() {
  ...
  return (
    <>
      <PageHeader
        eyebrow="مسترپلن فاز ۳/۲۰ — الگوی DemandFarm (نمای ده‌هزارپایی)"
        title="بریف فصلی خودکار (QBR)"
        description="مرور ۹۰ روزهٔ هر حساب از دادهٔ واقعی همان مستأجر — تحرک امتیازها، تعهدات، شکاف‌ها، رسانه و نظرسنجی‌ها در یک صفحهٔ قابل ارائه و چاپ. بدون LLM؛ همهٔ اعداد قابل ردیابی به رکورد منبع‌اند."
        actions={brief ? (
      ...
      {loading ? <Loading /> : (
        <>
          ...
          {!can('analytics.read') ? (
            <p className="pp-muted">برای مشاهدهٔ بریف فصلی مجوز «تحلیل‌ها» لازم است.</p>
          ) : briefLoading || !brief ? <Loading label="در حال تولید بریف…" /> : (
            <div className="qbr-sheet" aria-live="polite">
              <div className="qbr-head">
                ...
                <Badge tone="info">تولید خودکار · موتور قطعی</Badge>
              </div>
              <p className="qbr-headline">{brief.headline}</p>

              <div className="qbr-kpis">
                <div className="qbr-kpi"><span>میانگین سلامت روابط</span><strong>{brief.kpis.health.current != null ? fmtN(brief.kpis.health.current) : '—'}</strong><Delta current={k?.health.current ?? null} previous={k?.health.previous ?? null} /><small>{fmtN(brief.kpis.health.relationships)} رابطه</small></div>
                <div className="qbr-kpi"><span>تعاملات فصل</span><strong>{fmtN(k?.interactions.current ?? 0)}</strong><Delta current={k?.interactions.current ?? null} previous={k?.interactions.previous ?? null} /><small>فصل قبل: {fmtN(k?.interactions.previous ?? 0)}</small></div>
                <div className="qbr-kpi"><span>تعهدات باز / معوق</span><strong>{fmtN(k?.commitments.open ?? 0)} / {fmtN(k?.commitments.overdue ?? 0)}</strong>{(k?.commitments.overdue ?? 0) > 0 ? <span style={{ fontSize: 11, color: '#b91c1c' }}><AlertTriangle size={11} style={{ verticalAlign: '-1px' }} /> نیازمند پیگیری</span> : <span className="pp-muted" style={{ fontSize: 11 }}>بدون معوق</span>}<small>{fmtN(k?.commitments.fulfilledInPeriod ?? 0)} انجام‌شده در فصل</small></div>
                <div className="qbr-kpi"><span>شکاف‌های پوشش عمومی</span><strong>{k?.gaps.total != null ? fmtN(k.gaps.total) : '—'}</strong>{k?.gaps.critical != null && k.gaps.critical > 0 ? <span style={{ fontSize: 11, color: '#b45309' }}>{fmtN(k.gaps.critical)} بحرانی</span> : <span className="pp-muted" style={{ fontSize: 11 }}>{k?.gaps.note ?? 'بدون شکاف بحرانی'}</span>}<small>از برنامهٔ عموم‌ها</small></div>
                <div className="qbr-kpi"><span>ذکرهای رسانه‌ای</span><strong>{fmtN(k?.media.inPeriod ?? 0)}</strong><span className="pp-muted" style={{ fontSize: 11 }}>{fmtN(k?.media.positive ?? 0)} مثبت · {fmtN(k?.media.negative ?? 0)} منفی · {fmtN(k?.media.neutral ?? 0)} خنثی</span><small>رصد منابع منتخب</small></div>
                <div className="qbr-kpi"><span>نظرسنجی و موضع‌ها</span><strong>{fmtN(k?.surveys.answered ?? 0)}</strong><span className="pp-muted" style={{ fontSize: 11 }}>{fmtN(k?.surveys.stanceShifts ?? 0)} تغییر موضع</span><small>{fmtN(k?.surveys.createdInPeriod ?? 0)} نظرسنجی در فصل</small></div>
                <div className="qbr-kpi"><span>پورتال عمومی</span><strong>{fmtN(k?.portal.submissionsInPeriod ?? 0)}</strong><span className="pp-muted" style={{ fontSize: 11 }}>{fmtN(k?.portal.resolved ?? 0)} حل‌شده{k?.portal.avgResolutionDays != null ? ` · میانگین ${fmtN(k.portal.avgResolutionDays)} روز` : ''}</span><small>{(k?.portal.slaBreaches ?? 0) > 0 ? `${fmtN(k?.portal.slaBreaches ?? 0)} نقض SLA` : 'در چارچوب SLA'}</small></div>
                <div className="qbr-kpi"><span>تاریخ داده</span><strong style={{ fontSize: 13 }}>{faDate(brief._meta?.dataDate)}</strong><span className="pp-muted" style={{ fontSize: 11 }}>فقط دادهٔ واقعی این حساب</span><small>قابل ردیابی به رکورد منبع</small></div>
              </div>

              <div className="qbr-cols">
                <SectionCard title="تعهدات معوق" icon={<AlertTriangle size={15} />}>
                  ...
                  ) : <p className="pp-muted" style={{ fontSize: 12 }}>تعهد معوقی نیست.</p>}
                  {brief.sections.commitmentsDueSoon.length > 0 && (
                    <>
                      <p className="pp-muted" style={{ margin: '10px 0 4px', fontSize: 11.5 }}>نزدیک‌ترین سررسیدها:</p>
                ...
                <SectionCard title="شکاف‌های بحرانی پوشش" icon={<ClipboardList size={15} />}>
                  ...
                  ) : <p className="pp-muted" style={{ fontSize: 12 }}>{k?.gaps.note ?? 'شکاف بحرانی ثبت نشده است.'}</p>}
                </SectionCard>
                <SectionCard title="پوشش رسانه‌ای فصل" icon={<FileText size={15} />}>
                  ...
                  ) : <p className="pp-muted" style={{ fontSize: 12 }}>ذکر رسانه‌ای در این فصل شناسایی نشد (رصد محدود به منابع منتخب).</p>}
                  {brief.sections.stanceShifts.length > 0 && (
                    <>
                      <p className="pp-muted" style={{ margin: '10px 0 4px', fontSize: 11.5 }}>تغییر موضع‌های فصل:</p>
                      <ul style={{ margin: 0, paddingInlineStart: 16, display: 'grid', gap: 4, fontSize: 12 }}>
                        {brief.sections.stanceShifts.map((h, i) => <li key={i}>{h.memberName ?? h.memberId} — {h.fromStance} ← {h.toStance} ({h.cause === 'SURVEY' ? 'نظرسنجی' : h.cause})</li>)}
                      </ul>
                    </>
                  )}
                </SectionCard>
                <SectionCard title="توصیه‌های فصل آینده" icon={<TrendingUp size={15} />}>
                  <ol style={{ margin: 0, paddingInlineStart: 18, display: 'grid', gap: 6, fontSize: 12.5 }}>
                    {brief.recommendations.map((r, i) => <li key={i}>{r}</li>)}
                  </ol>
                </SectionCard>
              </div>
              <p className="pp-muted qbr-foot" style={{ fontSize: 10.5 }}>{brief._meta?.disclaimer} — تولیدشده در {new Date(brief._meta?.dataDate ?? Date.now()).toLocaleString('fa-IR')}</p>
            </div>
          )}
        </>
      )}
