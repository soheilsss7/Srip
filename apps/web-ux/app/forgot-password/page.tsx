...
export default function ForgotPassword(){
 ...
 async function submit(e:React.FormEvent){
  ...
  try{
   ...
   setMsg(t('اگر حساب وجود داشته باشد، درخواست بازیابی ثبت شد.'));
 ...
 return (
  <AuthShell>
    ...
    <p className="ac-sub">{t('ایمیل خود را وارد کنید؛ اگر حسابی وجود داشته باشد، لینک بازیابی برایتان ارسال می‌شود.')}</p>
    {msg ? (
      <div className="auth-form" style={{alignItems:'center',textAlign:'center',gap:12}}>
        <CheckCircle2 size={44} style={{color:'var(--srip-success)'}}/>
        <b>{t('درخواست ثبت شد')}</b>
      ...
      <form className="auth-form" onSubmit={submit} noValidate>
        <div className="field">
          <label className="field-label" htmlFor="fp-email">{t('ایمیل سازمانی')} <span className="req">*</span></label>
        ...
        <button className="btn btn-primary btn-block" disabled={busy||!email}>
          <KeyRound size={16}/> {busy?t('در حال ارسال…'):t('ارسال درخواست بازیابی')}
        </button>
      </form>
    )}
