...
const SECTIONS = [
  ...
  },
  {
    ...
    body: 'هاب «عموم‌ها» نقشهٔ ارتباط سازمان با ذی‌نفعان اثرگذار است: در «شناسنامهٔ سازمان» نوع شرکت و الگوی شش‌دسته‌ای (داخلی، نهادی و حاکمیتی، علمی و پژوهشی، اقتصادی و سرمایه‌گذاری، رسانه‌ای و عمومی، اکوسیستم فناوری و صنعت) تعیین می‌شود و در «گروه‌ها» همین فهرست ویژهٔ سازمان شما می‌شود. «اعضا و ارزیابی» هر عموم را با پیوند، مرحلهٔ بلوغ، موضع و قدرت و علاقه (ماتریس ۲×۲) ثبت می‌کند. «پوشش» خلأهای هر دسته را نشان می‌دهد و «شکاف‌ها و اقدام» شکاف‌های بحرانی را با مسیر پیشنهادی و خلاصهٔ مدیریتی ارائه می‌کند. خروجی‌ها در «خروجی و رسانه»: JSON، CSV و Excel. گره‌های گراف «شبکهٔ روابط» با رنگ دسته و حلقهٔ طلایی «خودِ شرکت» برچسب می‌خورند و با فیلتر دسته قابل تفکیک‌اند.',
  },
  {
  ...
  },
  {
    ...
    body: 'دستیار داخلی با ۹ قابلیت: جستجوی هوشمند، آمادگی جلسه، خلاصه‌سازی، استخراج اقدام و تعهد، تشخیص ریسک و فرصت، و «بهترین اقدام بعدی». همهٔ پاسخ‌ها از موتور قطعی درون‌سرور می‌آیند.',
  },
  {
...
const FAQS = [
  ...
  { q: 'داده‌ها بعد از ریستارت سرور می‌ماند؟', a: 'بله — داده‌ها روی دیسک ذخیره می‌شوند.' },
    ...
    <main className="feature-page">
      ...
      <section className="panel" id="quickstart">
        ...
        <div className="steps-row">
          <div className="step-card"><span className="step-num">۱</span><b>ورود با نقش مناسب</b><p>مالک (همه محدوده) یا مستأجر (محدودهٔ سازمان خودتان) — از صفحهٔ ورود با حساب کاربری خود وارد شوید.</p></div>
