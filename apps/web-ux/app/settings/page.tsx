...
const ROLE_LABELS: Record<string, string> = lt({
  SUPER_ADMIN: t('مدیر کل سیستم'), HOLDING_ADMIN: t('مدیر هلدینگ'), HOLDING_EXECUTIVE: t('مدیر ارشد هلدینگ'),
  SUBSIDIARY_ADMIN: t('مدیر شرکت'), SUBSIDIARY_EXECUTIVE: t('مدیر ارشد شرکت'), RELATIONSHIP_MANAGER: t('مدیر روابط'),
  PROJECT_MANAGER: t('مدیر پروژه'), ANALYST: t('تحلیلگر'), STANDARD_USER: t('کاربر استاندارد'), READ_ONLY: t('فقط خواندنی'),
});
const GROUP_FA: Record<string, string> = lt({
  General: t('کلی'), Core: t('هسته'), Admin: t('مدیریت'), Security: t('امنیت'), DataGovernance: t('حاکمیت داده'),
  Account: t('حساب‌ها'), Knowledge: t('دانش'), Intelligence: t('هوشمندی'), Reporting: t('گزارش‌ها'), Sales: t('فروش'),
});
const SCOPE_FA: Record<string, string> = lt({
  ALL: t('همهٔ محدوده'), ORGANIZATION: t('سازمان'), SUBSIDIARIES: t('زیرمجموعه‌ها'), DEPARTMENT: t('دپارتمان'),
  OWNED: t('تحت مالکیت'), SHARED: t('اشتراکی'), PRIVATE: t('خصوصی'),
});
...
export default function Settings() {
  ...
  async function savePrefs() {
    try {
      const localeChanged = getLocale() !== prefs.locale;
      ...
      if (localeChanged) {
        /* فاز ۴/۲۳: تغییر زبان = ذخیره + بارگذاری مجدد تا همهٔ ثابت‌های ماژول دوباره ارزیابی شوند */
        setLocale(prefs.locale === 'en' ? 'en' : 'fa');
        window.location.reload();
        return;
      }
      setNotice(t('ترجیحات رابط روی این مرورگر ذخیره شد.'));
    } catch (x) { setError((x as Error).message); }
  }
  ...
  async function verifyEnroll(e: FormEvent) {
    ...
    try {
      if (!device?.deviceId) throw new Error(t('ابتدا دستگاه ساخته شود.'));
      if (!/^\d{6}$/.test(code)) throw new Error(t('کد ۶ رقمی اپلیکشن احراز هویت را وارد کنید.'));
  ...
  async function testMfa(e: FormEvent) {
    ...
    try {
      if (!/^\d{6}$/.test(testCode)) throw new Error(t('کد ۶ رقمی وارد کنید.'));
      const out = await api<{ verified: boolean }>('/auth/mfa/verify', { method: 'POST', body: JSON.stringify({ code: testCode }) });
      if (!out.verified) throw new Error(t('کد نامعتبر است.'));
      setMfaMsg(t('کد دومرحله‌ای معتبر است — تأیید با موفقیت انجام شد.'));
  ...
  async function copyAll() {
    try {
      ...
      setMfaMsg(t('کدهای بازیابی کپی شدند — آن‌ها را در جای امن نگه دارید.'));
    } catch (x) { setError((x as Error).message); }
  }
  async function copySecret() {
    try {
      if (device?.secret) await navigator.clipboard.writeText(device.secret);
      setMfaMsg(t('کلید مخفی کپی شد.'));
    } catch (x) { setError((x as Error).message); }
  }
  ...
  return (
    <main className="feature-page">
      <PageHeader
        eyebrow={t('حساب کاربری')}
        title={t('تنظیمات کاربر')}
        description={t('پروفایل، عضویت‌های سازمانی و نقش‌ها، امنیت حساب (احراز دومرحله‌ای)، مجوزهای شما و ترجیحات رابط کاربری — دادهٔ پروفایل از سرور خوانده می‌شود.')}
        actions={
      ...
      {loading && !me ? <Loading label={t('در حال بارگذاری تنظیمات…')} /> : (
        <>
          <div className="grid2" style={{ alignItems: 'stretch' }}>
            {/* ------------------- profile card ------------------- */}
            <section className="panel" style={{ margin: 0 }}>
              <div className="panel-title">
                <div><h2 style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}><UserRound size={16} /> {t('پروفایل من')}</h2><p>{t('شناسه و وضعیت حساب بر پایهٔ سرور.')}</p></div>
              </div>
              <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                <span style={{ width: 62, height: 62, borderRadius: 18, display: 'grid', placeItems: 'center', flex: '0 0 auto', fontSize: 24, fontWeight: 900, background: 'linear-gradient(135deg, var(--srip-accent,#2563eb), var(--purple,#7c3aed))', color: '#fff' }}>
                  {(me?.name ?? t('؟')).trim().charAt(0)}
                </span>
                <span style={{ minWidth: 0 }}>
                  <span style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                    <b style={{ fontSize: 16 }}>{me?.name}</b>
                    {isOwner && <Badge tone="danger">{t('مالک سامانه')}</Badge>}
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 12 }}>
                <Link className="secondary-action" style={{ padding: '7px 12px', minHeight: 0, fontSize: 11.5 }} href="/sessions"><Smartphone size={13} /> {t('نشست‌ها و دستگاه‌ها')}</Link>
                <Link className="secondary-action" style={{ padding: '7px 12px', minHeight: 0, fontSize: 11.5 }} href="/privacy"><ShieldCheck size={13} /> {t('حقوق داده')}</Link>
                <Link className="secondary-action" style={{ padding: '7px 12px', minHeight: 0, fontSize: 11.5 }} href="/security"><Lock size={13} /> {t('امنیت')}</Link>
              </div>
            </section>
            ...
            <section className="panel" style={{ margin: 0 }}>
              <div className="panel-title">
                <div><h2 style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}><Building2 size={16} /> {t('عضویت‌های سازمانی')}</h2><p>{t('نقش، دپارتمان و محدودهٔ هر عضویت.')}</p></div>
              </div>
              {!me?.memberships?.length ? <div className="empty-state">{t('عضویتی ثبت نشده است.')}</div> : (
                    ...
                    <article className="listRow" key={m.id} style={{ alignItems: 'flex-start' }}>
                      ...
                      <span style={{ flex: 1, minWidth: 0 }}>
                        <span style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                          ...
                          <Badge tone={m.isPrimary ? 'success' : 'neutral'}>{m.isPrimary ? t('اصلی') : t('فرعی')}</Badge>
                        </span>
                        <span className="t-muted" style={{ display: 'block', fontSize: 10.5, marginTop: 1 }}>
                          نقش: {ROLE_LABELS[m.role] ?? m.role}
                          {m.department ? ` ${t('· دپارتمان:')} ${m.department}` : ''}
                        </span>
          ...
          <div className="grid2" style={{ alignItems: 'stretch' }}>
            ...
            <section className="panel" style={{ margin: 0 }}>
              <div className="panel-title">
                <div><h2 style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}><Fingerprint size={16} /> {t('امنیت حساب — تأیید دومرحله‌ای')}</h2><p>{t('هم‌ارزی با سرویس تأیید دومرحله‌ای واقعی: دستگاه‌های تأییدشده، کدهای بازیابی یک‌بارمصرف.')}</p></div>
              </div>
              {mfaRequired === null ? <Loading label={t('بررسی وضعیت MFA…')} /> : mfaRequired ? (
                <>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <span>
                      <b style={{ display: 'block', fontSize: 12.5 }}>{t('تأیید دومرحله‌ای فعال است')}</b>
                      <span className="t-muted" style={{ fontSize: 10.5 }}>{t('ورود به حساب به کد اپلیکشن احراز هویت نیاز دارد؛ در صورت گم‌شدن دستگاه، از کدهای بازیابی استفاده کنید.')}</span>
                    </span>
                  </div>
                  <button className="secondary-action" style={{ padding: '8px 14px', minHeight: 0 }} onClick={() => { setTestOpen(true); setError(''); }}><KeyRound size={13} /> {t('آزمون کد')}</button>
                </>
              ) : (
                <>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <span>
                      <b style={{ display: 'block', fontSize: 12.5 }}>{t('تأیید دومرحله‌ای غیرفعال است')}</b>
                      <span className="t-muted" style={{ fontSize: 10.5 }}>{t('فعال‌سازی برای محافظت از حساب در برابر ورود غیرمجاز توصیه می‌شود.')}</span>
                    </span>
                  </div>
                  <button className="btn btn-primary" style={{ padding: '8px 14px', minHeight: 0 }} onClick={() => { setEnrollOpen(true); setStep('form'); setDevice(null); setCode(''); setMfaMsg(''); setError(''); }}><Smartphone size={13} /> {t('فعال‌سازی')}</button>
                </>
              )}
            ...
            <section className="panel" style={{ margin: 0 }}>
              <div className="panel-title">
                <div><h2 style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}><BadgeCheck size={16} /> {t('مجوزهای من')}</h2><p>برگرفته از نقش‌ها و عضویت‌های شما؛ مجموع {me?.permissions?.length ?? 0} مجوز.</p></div>
              </div>
              {!me?.permissions?.length ? <div className="empty-state">{t('مجوزی ثبت نشده است.')}</div> : isOwner ? (
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '10px 12px', borderRadius: 10, background: 'color-mix(in srgb, var(--red,#dc2626) 6%, transparent)', border: '1px solid color-mix(in srgb, var(--red,#dc2626) 20%, transparent)' }}>
                  ...
                  <span style={{ fontSize: 12 }}><b>{t('دسترسی کامل (مالک)')}</b> {t('— همهٔ مجوزها بدون محدودیت، در همهٔ سازمان‌ها.')}</span>
                </div>
              ) : (
          ...
          <div className="grid2" style={{ alignItems: 'stretch' }}>
            ...
            <section className="panel" style={{ margin: 0 }}>
              <div className="panel-title">
                <div><h2 style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}><Monitor size={16} /> {t('ترجیحات رابط کاربری')}</h2><p>{t('این تنظیمات روی همین مرورگر (حافظهٔ محلی) نگهداری می‌شود و به حساب شما در سرور منتقل نمی‌گردد.')}</p></div>
              </div>
              <div className="entity-form" style={{ gap: 10 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <label className="field-label" style={{ fontSize: 11 }}>
                    ...
                    <select value={prefs.locale} onChange={e => setPrefs({ ...prefs, locale: e.target.value })}>
                      ...
                      <option value="en">{t('انگلیسی')}</option>
                    </select>
                  </label>
                  <label className="field-label" style={{ fontSize: 11 }}>
                    {t('منطقهٔ زمانی')}
                    <select value={prefs.timezone} onChange={e => setPrefs({ ...prefs, timezone: e.target.value })}>
                      <option value="Asia/Tehran">{t('تهران (Asia/Tehran)')}</option>
                      <option value="UTC">{t('هماهنگ جهانی (UTC)')}</option>
                      <option value="Europe/Berlin">{t('برلین (Europe/Berlin)')}</option>
                    </select>
                  </label>
                </div>
                <label className="field-label" style={{ fontSize: 11 }}>
                  {t('محدودهٔ پیش‌فرض در ورود')}
                  <select value={prefs.defaultScope} onChange={e => setPrefs({ ...prefs, defaultScope: e.target.value })}>
                    <option value="all">{t('همهٔ محدودهٔ مجاز')}</option>
                ...
                <button className="btn btn-primary" style={{ alignSelf: 'flex-start', padding: '8px 16px', minHeight: 0 }} onClick={savePrefs}><Save size={14} /> {t('ذخیرهٔ ترجیحات')}</button>
              </div>
            </section>
            ...
            <section className="panel" style={{ margin: 0 }}>
              <div className="panel-title">
                <div><h2 style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}><Link2 size={16} /> {t('دسترسی سریع')}</h2><p>{t('صفحات مرتبط با حساب، امنیت و حریم خصوصی شما.')}</p></div>
              </div>
              <div style={{ display: 'grid', gap: 7 }}>
                {[
                  ['/sessions', t('نشست‌ها و دستگاه‌های من'), t('مشاهده و لغو نشست‌های فعال'), <Smartphone key="i" size={14} />],
                  ['/security', t('امنیت و حاکمیت'), t('وضعیت امنیتی حساب و سامانه'), <Lock key="i" size={14} />],
                  ['/security-events', t('رویدادهای امنیتی'), t('ورودها و رخدادهای امنیتی'), <ScrollText key="i" size={14} />],
                  ['/privacy', t('حقوق داده و حریم خصوصی'), t('رضایت‌ها و درخواست‌های حق داده'), <ShieldCheck key="i" size={14} />],
                  ['/governance', t('وضعیت حاکمیت'), t('بررسی‌های مقدماتی حاکمیتی'), <CheckCircle2 key="i" size={14} />],
                  ['/enterprise', t('حاکمیت سازمانی'), t('خط‌مشی‌ها، پرچم‌ها و خروجی‌ها'), <Building2 key="i" size={14} />],
      ...
      <Modal
        open={enrollOpen}
        title={t('فعال‌سازی تأیید دومرحله‌ای')}
        description={t('دستگاه جدید ثبت می‌شود و پس از تأیید با یک کد ۶ رقمی، کدهای بازیابی یک‌بارمصرف صادر می‌گردد.')}
        onClose={() => setEnrollOpen(false)}
        footer={<>
          <button type="button" className="btn btn-secondary" onClick={() => setEnrollOpen(false)}><X size={14} /> {t('بستن')}</button>
          {step !== 'done' && (
            <button type="submit" form="mfa-enroll-form" className="btn btn-primary" disabled={busyOn('enroll') || busyOn('verify-enroll')}>
              {step === 'form' ? (busyOn('enroll') ? t('در حال ساخت…') : t('ساخت دستگاه')) : (busyOn('verify-enroll') ? t('در حال تأیید…') : t('تأیید و دریافت کدهای بازیابی'))}
            </button>
          )}
        </>}
      >
          ...
          <form id="mfa-enroll-form" className="entity-form" onSubmit={verifyEnroll}>
            <div style={{ display: 'grid', gap: 7 }}>
              <p style={{ fontSize: 11, margin: 0 }} className="t-muted">
                {t('کلید مخفی را در اپلیکشن وارد کنید (یا روی «کپی» بزنید) سپس کد ۶ رقمی تولیدشده را این‌جا وارد کنید:')}
              </p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'var(--card-bg-soft,#f1f5f9)', borderRadius: 8, padding: '7px 9px', direction: 'ltr' }}>
                ...
                <button type="button" className="secondary-action" style={{ padding: '5px 9px', minHeight: 0 }} onClick={copySecret}><Copy size={12} /> {t('کپی')}</button>
              </div>
              <div style={{ direction: 'ltr', textAlign: 'left' }}>
                <code style={{ fontSize: 9.5, opacity: .75, wordBreak: 'break-all' }}>{device.otpauthUrl}</code>
              </div>
              <label className="field-label" style={{ fontSize: 11 }}>{t('کد ۶ رقمی')} <span className="req">*</span>
        ...
        {step === 'done' && (
          <div style={{ display: 'grid', gap: 10 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '10px 12px', borderRadius: 10, background: 'color-mix(in srgb, var(--green,#16a34a) 7%, transparent)', border: '1px solid color-mix(in srgb, var(--green,#16a34a) 25%, transparent)' }}>
              ...
              <span style={{ fontSize: 11.5 }}><b>{t('تأیید دومرحله‌ای فعال شد.')}</b> {t('از این پس ورود به کد ۶ رقمی نیاز دارد.')}</span>
            </div>
            {recoveryCodes.length > 0 && (
              <>
                <p style={{ fontSize: 11, margin: 0 }} className="t-muted">
                  <AlertTriangle size={12} style={{ verticalAlign: -2, color: 'var(--gold,#d97706)' }} /> {t('کدهای بازیابی زیر')} <b>{t('فقط همین یک بار')}</b> نمایش داده می‌شوند و هر کدام یک‌بار مصرف‌اند — آن‌ها را ذخیره کنید:
                </p>
                ...
                <button className="secondary-action" style={{ padding: '7px 12px', minHeight: 0, justifySelf: 'flex-start' }} onClick={copyAll}><Copy size={13} /> {t('کپی همهٔ کدها')}</button>
              </>
            )}
          </div>
        )}
      ...
      <Modal
        open={testOpen}
        title={t('آزمون کد دومرحله‌ای')}
        description={t('یک کد ۶ رقمی از اپلیکشن احراز هویت وارد کنید تا مسیر تأیید (mfa/verify) آزمایش شود.')}
        onClose={() => setTestOpen(false)}
        footer={<>
          <button type="button" className="btn btn-secondary" onClick={() => setTestOpen(false)}><X size={14} /> {t('انصراف')}</button>
          <button type="submit" form="mfa-test-form" className="btn btn-primary" disabled={busyOn('test')}>{busyOn('test') ? t('در حال بررسی…') : t('تأیید کد')}</button>
        </>}
      >
        <form id="mfa-test-form" className="entity-form" onSubmit={testMfa}>
          <label className="field-label">{t('کد ۶ رقمی')} <span className="req">*</span>
