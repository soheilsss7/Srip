    ...
    <main className="feature-page">
      ...
      {loading ? (
        ...
        <div className="stat-grid">
          <StatCard icon={<Siren size={18} />} label="بحرانی — اقدام فوری" value={counts.CRITICAL} iconClass="ic-red" sub={counts.CRITICAL > 0 ? 'همین امروز ببندید' : 'مورد بحرانی ندارید'} />
          <StatCard icon={<AlertTriangle size={18} />} label="هشدار" value={counts.WARNING} iconClass="ic-gold" sub="در حال تبدیل‌شدن به بحران" />
          <StatCard icon={<Info size={18} />} label="اطلاع — بازبینی" value={counts.INFO} iconClass="ic-blue" sub="ثبت نتیجه / بررسی" />
          <StatCard icon={<CheckCheck size={18} />} label="کل فعال" value={total} iconClass="ic-teal" sub="پس از اقدام، «حل شد» بزنید" />
        </div>
        ...
        <SectionCard title="پالایش بر اساس ماژول" icon={<Activity size={17} />} description="هر ماژول همان منطق تشخیص قبلیِ خودش را دارد؛ فقط خروجی یکجا جمع می‌شود.">
          <div className="chip-row" style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
            <button className={`chip ${moduleFilter === '' ? 'purple' : 'neutral'}`} style={{ cursor: 'pointer', border: 'none' }} onClick={() => setModuleFilter('')}>همه ({total})</button>
            {modules.map(m => (
              <button key={m.id} className={`chip ${moduleFilter === m.id ? 'purple' : 'neutral'}`} style={{ cursor: 'pointer', border: 'none' }} onClick={() => setModuleFilter(moduleFilter === m.id ? '' : m.id)}>
                {MODULE_ICON[m.id]} {m.fa} ({counts.byModule?.[m.id] ?? 0})
              </button>
            ))}
            <span style={{ flex: 1 }} />
        ...
        <SectionCard
          title="هشدارهای فعال"
          icon={<Siren size={17} />}
          description={items.length ? `${items.length} مورد در محدودهٔ دسترسی شما` : undefined}
        >
          {items.length === 0 ? (
