  ...
  return (
    <main className="error-page" style={{minHeight:'60vh',display:'grid',placeItems:'center',padding:24}}>
      <section className="route-error" style={{maxWidth:460,textAlign:'center',display:'flex',flexDirection:'column',alignItems:'center',gap:14}}>
        ...
        <div>
          <div className="eyebrow" style={{textAlign:'center'}}>{t('خطای مسیر')}</div>
          <strong style={{fontSize:22,display:'block',marginTop:4}}>{t('خطای غیرمنتظره')}</strong>
          <p style={{marginTop:8}}>{t('تغییرات ذخیره‌نشده را بررسی کنید و دوباره تلاش کنید.')}</p>
          {error?.digest && <code className="chip neutral" dir="ltr" style={{marginTop:4}}>{error.digest}</code>}
        </div>
        <button className="btn btn-primary" onClick={reset}><RefreshCw size={16}/> {t('بازنشانی صفحه')}</button>
      </section>
    </main>
  );
}
