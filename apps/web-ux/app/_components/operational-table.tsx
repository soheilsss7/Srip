  ...
  return <main className="feature-page">
    <PageHeader eyebrow={config.eyebrow} title={config.title} description={config.description} actions={
      <div className="toolbar">
        {config.create&&<button className="btn btn-primary" onClick={()=>{setError('');setForm({});setOpen(true)}}><Plus size={15}/> {t('ایجاد')}</button>}
    ...
    <section className="section-card">
      <div className="section-head">
        <div><h2>{t('داده‌ها')}</h2><p>{t('داده فقط از API مجاز سرور خوانده می‌شود.')}</p></div>
        {config.searchParam&&(
          <div className="toolbar-search">
            <Search size={15}/>
            <input aria-label={t('جستجو')} placeholder={t('جستجو…')} value={query} onChange={e=>setQuery(e.target.value)} style={{minHeight:36,fontSize:12.5}}/>
          </div>
        )}
      </div>
      {loading?<Loading/>:rows.length?<DataTable columns={cols.map(k=>({key:k,label:config.labels?.[k]??k}))} rows={rows.map(r=>Object.fromEntries(cols.map(k=>[k,display(k,r[k])])))}/>:<Empty>{t('داده‌ای برای این Scope وجود ندارد.')}</Empty>}
    </section>
   <Modal open={open} title={t('ایجاد')} description={t('اطلاعات جدید را وارد کنید؛ اعتبارسنجی نهایی در سرور انجام می‌شود.')} onClose={()=>setOpen(false)}
     footer={<>
       <button className="btn btn-secondary" type="button" onClick={()=>setOpen(false)}><X size={14}/> {t('انصراف')}</button>
       <button className="btn btn-primary" type="submit" form="op-modal-form" disabled={saving}>{saving?t('در حال ذخیره…'):t('ذخیره')}</button>
     </>}>
       ...
       {config.create?.fields.map(f=>(
         <div className={`field ${f.type==='textarea'||f.type==='select'?'full':''}`} key={f.name}>
           ...
           {f.type==='textarea'
             ...
             : f.type==='select'
               ? <select value={form[f.name]??''} onChange={e=>setForm({...form,[f.name]:e.target.value})} required={f.required}><option value="">{t('انتخاب کنید')}</option>{f.options?.map(o=><option key={o} value={o}>{fa(o)}</option>)}</select>
