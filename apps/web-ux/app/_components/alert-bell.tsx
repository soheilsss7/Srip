...
export function AlertBell() {
  ...
  return (
    <Link
      ...
      href="/alerts"
      title={critical != null && critical > 0 ? `هشدارهای یکپارچه — ${critical} مورد بحرانی` : 'هشدارهای یکپارچه — همهٔ سیگنال‌ها در یک نگاه'}
      aria-label={critical != null && critical > 0 ? `هشدارها: ${critical} مورد بحرانی` : 'هشدارها'}
    >
      <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        <path d="M12 2 L13.5 8.5 L20 10 L13.5 11.5 L12 18 L10.5 11.5 L4 10 L10.5 8.5 Z" />
      </svg>
      {critical != null && critical > 0 && (
        <span className="alert-badge" aria-hidden="true">{critical > 99 ? '99+' : critical}</span>
      )}
    </Link>
  );
}
