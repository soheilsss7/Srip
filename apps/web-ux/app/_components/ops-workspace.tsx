...
const cols=rows[0]?Object.keys(rows[0]).filter(k=>!k.startsWith('_')).slice(0,8):[];
 ...
 return <main className="feature-page"><PageHeader eyebrow={eyebrow} title={title} description={description} actions={<div className="toolbar"><button onClick={load}>{t('بازخوانی')}</button>{actions.map(a=><button key={a.label} onClick={()=>act(a)} disabled={!!busy}>{busy===a.label?t('در حال اجرا…'):a.label}</button>)}</div>}/><ErrorCard message={error}/>{loading?<Loading/>:rows.length?<DataTable columns={cols.map(k=>({key:k,label:labelKey(k)}))} rows={rows.map((r:any)=>Object.fromEntries(cols.map(k=>[k, r[k]==null?'—':typeof r[k]==='object'?JSON.stringify(r[k]):(ENUM_COLS.has(k)?fa(r[k]):String(r[k]))])))}/>:<section className="panel"><Empty>{t('داده‌ای برای این Scope در دسترس نیست.')}</Empty></section>}</main>
}
