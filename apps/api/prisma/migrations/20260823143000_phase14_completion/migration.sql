...
CREATE INDEX IF NOT EXISTS "Relationship_search_fts_idx" ON "Relationship" USING GIN (to_tsvector('simple', coalesce("relationshipType",'') || ' ' || coalesce("status",'')));
...
CREATE INDEX IF NOT EXISTS "Opportunity_search_fts_idx" ON "Opportunity" USING GIN (to_tsvector('simple', coalesce("name",'') || ' ' || coalesce("description",'') || ' ' || coalesce("status",'')));
