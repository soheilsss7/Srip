...
export class DataLifecycleService {
  ...
  private async load(entityType:string,id:string,includeDeleted=true){const row=await this.delegate(entityType).findUnique({where:{id}});if(!row||(!includeDeleted&&row.deletedAt))throw new NotFoundException(`${entityType} not found`);return EntityResponseDto.fromUnknown(row);}
