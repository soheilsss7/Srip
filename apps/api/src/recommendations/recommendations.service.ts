...
export class RecommendationsService {
  ...
  private async relationshipScope(userId:string, relationshipId:string) {
    ...
    // Mock-contract compatibility: a relationship is visible when the actor has
    // scope over EITHER side (the tenant sees its own organization plus the
    // counterparties of its relationships). Both-side enforcement would hide
    // legitimate tenant data and is not part of the established contract.
    await this.authorization.assertAnyOrganizationAccess(userId, [r.sourceOrganizationId, r.targetOrganizationId]);
    await this.authorization.assertPermission(userId, 'relationship.read', { organizationId: r.sourceOrganizationId }).catch(async () => {
