...
export class SearchService {
  ...
  private async ftsIds(table: string, q: string, organizationId?: string, organizationIds?: string[] | null): Promise<string[]> {
    ...
    const queries: Record<string, any> = {
      ...
      Relationship: this.prisma.$queryRaw`SELECT id FROM "Relationship" WHERE "deletedAt" IS NULL ${rel} AND to_tsvector('simple', coalesce("relationshipType",'') || ' ' || coalesce("status"::text,'')) @@ plainto_tsquery('simple', ${q}) ORDER BY "id" LIMIT 100`,
      ...
      Opportunity: this.prisma.$queryRaw`SELECT id FROM "Opportunity" WHERE "deletedAt" IS NULL ${org} AND to_tsvector('simple', coalesce("name",'') || ' ' || coalesce("description",'') || ' ' || coalesce("status"::text,'')) @@ plainto_tsquery('simple', ${q}) ORDER BY "id" LIMIT 100`,
