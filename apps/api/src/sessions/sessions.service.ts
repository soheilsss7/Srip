...
export class SessionsService {
  ...
  async listForAdmin(userId: string) { return EntityResponseDto.manyUnknown(await this.prisma.session.findMany({ where: { userId }, select: { id: true, deviceName: true, ipAddress: true, userAgent: true, expiresAt: true, idleExpiresAt: true, absoluteExpiresAt: true, lastActivityAt: true, revokedAt: true, rotatedAt: true, createdAt: true }, orderBy: { createdAt: 'desc' }, take: 100 })); }
}
