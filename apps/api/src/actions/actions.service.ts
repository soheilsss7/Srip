...
export class ActionsService {
  ...
  async list(userId: string, page?: string, pageSize?: string) {
    ...
    const where: Prisma.ActionWhereInput = { deletedAt: null, ...(ids ? { OR: [{ ownerId: userId }, { organizationId: { in: ids } }, { relationship: { OR: [{ sourceOrganizationId: { in: ids } }, { targetOrganizationId: { in: ids } }] } }, { meeting: { organizationId: { in: ids } } }, { project: { organizationId: { in: ids } } }] } : { ownerId: userId }) };
  ...
  async listOverdue(userId: string, page?: string, pageSize?: string) { const ids = await this.authorization.accessibleOrganizationIds(userId); const p = parsePagination(page, pageSize); const where: Prisma.ActionWhereInput = { deletedAt: null, status: { in: ['OPEN', 'IN_PROGRESS', 'BLOCKED'] }, dueAt: { lt: new Date() }, ...(ids ? { OR: [{ ownerId: userId }, { organizationId: { in: ids } }, { relationship: { OR: [{ sourceOrganizationId: { in: ids } }, { targetOrganizationId: { in: ids } }] } }] } : { ownerId: userId }) }; const [items,total]=await this.prisma.$transaction([this.prisma.action.findMany({where,orderBy:{dueAt:'asc'},skip:p.skip,take:p.take}),this.prisma.action.count({where})]); return {items:EntityResponseDto.many('Action',items),page:p.page,pageSize:p.pageSize,total,totalPages:Math.ceil(total/p.pageSize)}; }
  async listDueSoon(userId: string, days = 7, page?: string, pageSize?: string) { const ids = await this.authorization.accessibleOrganizationIds(userId); const p = parsePagination(page,pageSize); const horizon = new Date(Date.now()+Math.max(1,days)*86400000); const where: Prisma.ActionWhereInput={deletedAt:null,status:{in:['OPEN','IN_PROGRESS','BLOCKED']},dueAt:{gte:new Date(),lte:horizon},...(ids?{OR:[{ownerId:userId},{organizationId:{in:ids}},{relationship:{OR:[{sourceOrganizationId:{in:ids}},{targetOrganizationId:{in:ids}}]}}]}:{ownerId:userId})}; const [items,total]=await this.prisma.$transaction([this.prisma.action.findMany({where,orderBy:{dueAt:'asc'},skip:p.skip,take:p.take}),this.prisma.action.count({where})]); return {items:EntityResponseDto.many('Action',items),page:p.page,pageSize:p.pageSize,total,totalPages:Math.ceil(total/p.pageSize)}; }
}
