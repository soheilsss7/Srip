...
export class RelationshipsService {
  constructor(private readonly prisma: PrismaService, private readonly authorization: AuthorizationService, private readonly audit: AuditService, private readonly eventBus: EventBusService, private readonly lifecycle: DataLifecycleService, private readonly presenter: RelationshipPresenter, private readonly approvals: ApprovalService, private readonly criteria: CriteriaService) {}

  async list(userId: string, organizationId?: string, status?: any, lifecycleStage?: RelationshipLifecycleStage, page = 1, pageSize = 50, marketKind?: string, isMarketEntry?: string, marketSegment?: string) {
    ...
    const marketKindFilter = marketKind && ['MARKET','NON_MARKET','HYBRID'].includes(String(marketKind)) ? { marketKind: marketKind as any } : {};
    const isEntryFilter = isMarketEntry === 'true' ? { isMarketEntry: true } : isMarketEntry === 'false' ? { isMarketEntry: false } : {};
    const segmentFilter = marketSegment ? { marketSegment: { contains: String(marketSegment), mode: 'insensitive' as const } } : {};
    const whereBase: any = {
        deletedAt: null,
    ...
    const [rows,total] = await Promise.all([this.prisma.relationship.findMany({
      ...
      take: pageSize,
    }), this.prisma.relationship.count({ where: whereBase })]);
    const presented = await this.presenter.presentMany(userId, rows);
    const criteriaMap = await this.criteria.summaries('RELATIONSHIP', presented.map((r: any) => String(r.id)));
    return { data: presented.map((r: any) => ({ ...r, criteria: criteriaMap.get(String(r.id)) ?? null })), page, pageSize, total, totalPages: Math.ceil(total / pageSize) };
  }
  ...
  async get(userId: string, id: string) {
    ...
    const presented = await this.presenter.present(userId, r);
    const criteriaMap = await this.criteria.summaries('RELATIONSHIP', [id]);
