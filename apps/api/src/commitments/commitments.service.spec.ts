...
describe('CommitmentsService Phase 9 contracts', () => {
  ...
  const eventBus: any = { transaction: jest.fn((cb: any) => cb(prisma)), publishInTransaction: jest.fn() };
  const service = new CommitmentsService(prisma, authorization, audit, notifications, eventBus, {} as any);
...
describe('CommitmentsService.sweepOverdue (Phase 26 automatic follow-up)', () => {
  it('transitions every past-due OPEN commitment to OVERDUE and notifies its owner', async () => {
    ...
    const prisma: any = {
      commitment: {
        ...
        findUniqueOrThrow: jest.fn().mockImplementation(({ where }) => Promise.resolve({ id: where.id, status: 'OVERDUE' })),
      },
    };
    ...
    expect(prisma.commitment.updateMany).toHaveBeenCalledTimes(2);
  ...
  it('does not fail the sweep if a single notification delivery throws', async () => {
    ...
    const prisma: any = { commitment: { findMany: jest.fn().mockResolvedValue(due), update: jest.fn().mockResolvedValue({ id: 'c1', status: 'OVERDUE' }), updateMany: jest.fn().mockResolvedValue({ count: 1 }), findUniqueOrThrow: jest.fn().mockResolvedValue({ id: 'c1', status: 'OVERDUE' }) } };
    const authorization: any = {};
    const audit: any = { logMutation: jest.fn() };
...
describe('CommitmentsService.listOverdue / listDueSoon (Phase 26 follow-up views)', () => {
  it('scopes listOverdue to OVERDUE status only', async () => {
    const prisma: any = { commitment: { findMany: jest.fn().mockResolvedValue([]), count: jest.fn().mockResolvedValue(0) }, $transaction: jest.fn((queries: any[]) => Promise.all(queries)) };
