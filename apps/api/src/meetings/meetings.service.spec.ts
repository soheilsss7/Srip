...
describe('MeetingsService.extractActionItems (deterministic, no AI/network dependency)', () => {
  ...
  const audit: any = {};
  const service = new MeetingsService(prisma, authorization, audit, { transaction: jest.fn((cb: any) => cb(prisma)), publishInTransaction: jest.fn() } as any, { softDelete: jest.fn() } as any);
