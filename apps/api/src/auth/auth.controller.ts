import { Body, Controller, Post, Req, UseGuards, Get, Query, HttpCode } from '@nestjs/common';
...
export class AuthController {
  ...
  @Post('login') @HttpCode(200) login(@Body() d: AuthDto, @Req() r: any) { return this.auth.login(d.email, d.password, { ip: r.ip, userAgent: r.headers['user-agent'], otp: d.otp }); }
  @Post('refresh') @HttpCode(200) refresh(@Body() d: TokenDto, @Req() r: any) { return this.auth.refresh(d.token, { ip: r.ip, userAgent: r.headers['user-agent'] }); }
  @Post('logout') @HttpCode(200) logout(@Body() d: TokenDto) { return this.auth.logout(d.token); }
