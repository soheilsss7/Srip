...
export default function Intelligence(){
 ...
 const label=(n:any,fallback='رکورد مرتبط')=>n?.label??n?.name??n?.displayName??n?.title??(n?'رکورد مرتبط':fallback);
 ...
 <section className="dashboard-grid">
  <article className="panel"><h2>Risk Signals</h2>{risk.length===0?<Empty>سیگنال ریسکی در Scope شما نیست.</Empty>:<DataTable columns={[{key:'relationship',label:'رابطه مرتبط'},{key:'riskScore',label:'امتیاز ریسک'},{key:'healthScore',label:'سلامت'},{key:'resilienceScore',label:'تاب‌آوری'},{key:'signals',label:'سیگنال‌ها'}]} rows={risk.map(r=>({relationship:label(r.relationship),riskScore:<Badge tone={r.riskScore>=70?'danger':r.riskScore>=40?'warning':'success'}>{r.riskScore}</Badge>,healthScore:r.healthScore??'—',resilienceScore:r.resilienceScore??'—',signals:Array.isArray(r.signals)?r.signals.map((s:string)=><span key={s} className="pill"> {s} </span>):r.signals}))}/>}</article>
  <article className="panel"><h2>Opportunity Detection</h2>{opps.length===0?<Empty>فرصت قابل‌تشخیصی وجود ندارد.</Empty>:<DataTable columns={[{key:'relationship',label:'رابطه مرتبط'},{key:'type',label:'نوع'},{key:'confidence',label:'اطمینان'},{key:'reason',label:'دلیل'},{key:'sourceOrganization',label:'سازمان مبدأ'},{key:'targetOrganization',label:'سازمان مقصد'}]} rows={opps.map(o=>({relationship:label(o.relationship),type:o.type??'—',confidence:<Badge tone={o.confidence>=70?'success':o.confidence>=40?'warning':'info'}>{pct(o.confidence)}</Badge>,reason:o.reason??'—',sourceOrganization:label(o.sourceOrganization,'سازمان مبدأ'),targetOrganization:label(o.targetOrganization,'سازمان مقصد')}))}/>}</article>
 </section>
