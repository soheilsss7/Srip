...
export default function Page({params}:{params:Promise<{id:string}>}){
 const {id}=use(params);
 const [r,setR]=useState<any>(null),[t,setT]=useState<any>(null),[error,setError]=useState(''),[busy,setBusy]=useState('');
 const load=useCallback(async()=>{setError('');try{const [a,b]=await Promise.all([api(`/relationships/${id}`),api(`/relationships/${id}/timeline`)]);setR(a);setT(b)}catch(e){setError((e as Error).message)}},[id]);
 useEffect(()=>{load()},[load]);
 ...
 return <main className="feature-page">
  <PageHeader eyebrow="CORE DOMAIN · RELATIONSHIP PROFILE" title={r?`${r.sourceOrganization?.name} ↔ ${r.targetOrganization?.name}`:'Relationship'} description={`${r?.relationshipType??''} · ${r?.status??''}`} actions={<div className="toolbar"><button className="secondary-action" onClick={load} disabled={!!busy}>بازخوانی</button><label className="inline-label">وضعیت<select value={r?.status??'ACTIVE'} disabled={!!busy} onChange={e=>doIt('status',()=>api(`/relationships/${id}`,{method:'PATCH',body:JSON.stringify({status:e.target.value})}))}>{STATUS.map(s=><option key={s} value={s}>{s}</option>)}</select></label><label className="inline-label">مرحله چرخه زندگی<select value={r?.lifecycleStage??'ACTIVE'} disabled={!!busy} onChange={e=>doIt('lifecycle',()=>api(`/relationships/${id}/lifecycle`,{method:'PATCH',body:JSON.stringify({lifecycleStage:e.target.value})}))}>{LIFECYCLE.map(s=><option key={s} value={s}>{s}</option>)}</select></label><button className="primary-action" disabled={!!busy} onClick={()=>doIt('recalc',()=>api(`/relationships/${id}/recalculate-score`,{method:'POST'}))}>محاسبه مجدد امتیاز</button>{r?.status==='ARCHIVED'?<button className="primary-action" disabled={!!busy} onClick={()=>doIt('restore',()=>api(`/relationships/${id}/restore`,{method:'POST'}))}>بازیابی</button>:<button className="danger-action" disabled={!!busy} onClick={()=>{if(confirm('این رابطه بایگانی شود؟'))doIt('archive',()=>api(`/relationships/${id}/archive`,{method:'PATCH'}))}}>بایگانی</button>}</div>}/>
