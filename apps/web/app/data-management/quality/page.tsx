...
export default function DataQuality() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [quality, duplicateReport] = await Promise.all([api(`/data/quality${query}`), api(`/data/duplicates${query}`)]);
      setData(quality);
  ...
  async function previewMerge(primaryId: string, duplicateId: string) {
    ...
    try {
      setMergePreview(await api('/data/duplicates/merge-preview', { method: 'POST', body: JSON.stringify({ entityType: 'ORGANIZATION', primaryId, duplicateId, ...(scopeId !== 'all' ? { organizationId: scopeId } : {}) }) }));
    } catch (x) { setError((x as Error).message); }
  ...
  async function mergeConfirmed() {
    ...
    try {
      await api('/data/duplicates/merge', { method: 'POST', body: JSON.stringify({ entityType: mergePreview.entityType, primaryId: mergePreview.primary.id, duplicateId: mergePreview.duplicate.id, confirmation, ...(scopeId !== 'all' ? { organizationId: scopeId } : {}) }) });
