...
function Score({ value, label }: { value: number | undefined; label: string }) {
  ...
  return (
    <div className="score-block">
      ...
      <div className="score-track"><span className={`score-fill ${toneClass(scoreTone(v))}`} style={{ width: `${v}%` }} /></div>
    </div>
  );
}
...
function FunnelVisual({ stages, conversion }: { stages?: Record<string, number>; conversion?: Record<string, number> }) {
  ...
  return (
    <div className="funnel">
      ...
      <div className="funnel-caption">نسبت‌ها بر پایهٔ «دیده شده» (۱۰۰٪) محاسبه شده‌اند.</div>
    </div>
  );
}
  ...
  return (
    <div className="dashboard-page dash">
      ...
      <div className="page-heading">
        <div>
          <div className="eyebrow">مدیریت ارشد / هوشمندی راهبردی</div>
          <h1>{me?.name ? `سلام، ${me.name}` : 'سلام'}</h1>
          <p className="subtitle">مرکز فرمان یکپارچه — شاخص‌ها زنده از سرور، بر اساس نقش، مجوز و محدودهٔ سازمانی شما.</p>
        </div>
