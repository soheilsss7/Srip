...
const [linkDepId,setLinkDepId]=useState('');
 ...
 return <main className="feature-page">
  ...
  {!a&&!error?<Loading/>:a&&<>
   ...
   <div className="split-panels">
    <section className="panel"><div className="panel-title"><div><h2>وابستگی‌ها (این اقدام وابسته است به…)</h2><Badge>{deps.length}</Badge></div>{linkDepId||linkDepId!==''?<span className="inline-form"><input className="inline-input" value={linkDepId} onChange={e=>setLinkDepId(e.target.value)} placeholder="Action ID" onKeyDown={e=>{if(e.key==='Enter'&&linkDepId.trim()){addDependency(linkDepId.trim());setLinkDepId('')}}}/><button className="primary-action" onClick={()=>{if(linkDepId.trim()){addDependency(linkDepId.trim());setLinkDepId('')}}}>افزودن</button></span>:<button className="secondary-action" onClick={()=>setLinkDepId(' ')} disabled={!!busy}>افزودن وابستگی</button>}</div>{deps.length?<div className="list">{deps.map((d:any)=><div className="listRow" key={d.id??d.dependsOnActionId}><Badge tone={d.dependsOnAction?.status==='DONE'?'success':'neutral'}>{d.dependsOnAction?.status??'—'}</Badge><span><strong>{d.dependsOnAction?.title??d.dependsOnActionId}</strong><small>{d.dependsOnAction?.dueAt?`سررسید ${new Date(d.dependsOnAction.dueAt).toLocaleDateString()}`:'بدون سررسید'}</small></span><span className="resource-row-actions"><button onClick={()=>removeDependency(d.dependsOnActionId)}>حذف</button></span></div>)}</div>:<p className="empty-state">وابستگی ثبت نشده است.</p>}</section>
