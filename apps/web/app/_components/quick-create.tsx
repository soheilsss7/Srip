...
type Entity={key:string;label:string;endpoint:string;fields:{name:string;label:string;type?:string;required?:boolean;entityEndpoint?:string}[]};
const entities:Entity[]=[
 {key:'organization',label:'سازمان',endpoint:'/organizations',fields:[{name:'name',label:'نام',required:true},{name:'type',label:'نوع'}]},
 {key:'person',label:'شخص',endpoint:'/people',fields:[{name:'firstName',label:'نام',required:true},{name:'lastName',label:'نام خانوادگی',required:true},{name:'organizationId',label:'سازمان',required:true,entityEndpoint:'/organizations'},{name:'email',label:'ایمیل',type:'email'}]},
...
export function QuickCreate({open,onClose}:{open:boolean;onClose:()=>void}){
 ...
 return <div className="quick-overlay"><section className="quick-card"><header><div><span className="eyebrow">QUICK ACTION</span><h2>ایجاد سریع</h2></div><button onClick={onClose}>×</button></header><div className="quick-types">{entities.map(x=><button className={x.key===entity.key?'active':''} onClick={()=>{setEntity(x);setV({})}} key={x.key}>{x.label}</button>)}</div><form className="entity-form" onSubmit={submit}>{entity.fields.map(f=>f.entityEndpoint?<EntityPicker key={f.name} label={f.label} value={v[f.name]??''} onChange={value=>setV({...v,[f.name]:value})} endpoint={f.entityEndpoint} required={f.required} disabled={busy} scopeId={scopeId}/>:<label key={f.name}>{f.label}<input type={f.type??'text'} required={f.required} value={v[f.name]??''} onChange={e=>setV({...v,[f.name]:e.target.value})}/></label>)}<button className="primary-action" disabled={busy}>{busy?'در حال ثبت…':'ایجاد '+entity.label}</button></form>{msg&&<div className="status-message">{msg}</div>}</section></div>
}