...
export function QuickCreate({ open, onClose, onCreated, context }: { open: boolean; onClose: () => void; onCreated?: () => void; context?: Context }) {
  ...
  return <div className="quick-overlay" role="dialog" aria-modal="true" aria-label="ایجاد سریع"><section className="quick-card"><header><div><span className="eyebrow">QUICK ACTION</span><h2>ایجاد سریع</h2></div><button type="button" onClick={onClose} aria-label="بستن">×</button></header><div className="quick-types">{entities.map(item => <button type="button" disabled={busy} className={item.key === entity.key ? 'active' : ''} onClick={() => selectEntity(item)} key={item.key}>{item.label}</button>)}</div>{!writable&&<div className="notice">مجوز ایجاد «{entity.label}» برای شما فعال نیست.</div>}<form className="entity-form" onSubmit={submit}>{entity.fields.map(field => field.entityEndpoint ? <EntityPicker key={field.name} label={field.label} value={values[field.name] ?? ''} onChange={value => setValues({ ...values, [field.name]: value })} endpoint={field.entityEndpoint} required={field.required} disabled={busy||!writable} scopeId={scopeId} /> : field.type === 'textarea' ? <label key={field.name}>{field.label}<textarea required={field.required} value={values[field.name] ?? ''} onChange={event => setValues({ ...values, [field.name]: event.target.value })} disabled={busy||!writable} /></label> : field.type === 'select' ? <label key={field.name}>{field.label}<select required={field.required} value={values[field.name] ?? ''} onChange={event => setValues({ ...values, [field.name]: event.target.value })} disabled={busy||!writable}><option value="">انتخاب کنید</option>{field.options?.map(option => <option key={option} value={option}>{option}</option>)}</select></label> : <label key={field.name}>{field.label}<input type={field.type ?? 'text'} required={field.required} value={values[field.name] ?? ''} onChange={event => setValues({ ...values, [field.name]: event.target.value })} disabled={busy||!writable} /></label>)}<button className="primary-action" disabled={busy||!writable}>{busy ? 'در حال ثبت…' : `ایجاد ${entity.label}`}</button></form>{message && <div className="status-message" role="status">{message}</div>}</section></div>;
}
