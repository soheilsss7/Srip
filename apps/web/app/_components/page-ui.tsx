...
function tableValue(value:any):React.ReactNode{
 if(value===null||value===undefined||value==='')return '—';
 if(React.isValidElement(value))return value;
 if(Array.isArray(value))return value.length?value.map((item,index)=><React.Fragment key={index}>{index?'، ':''}{tableValue(item)}</React.Fragment>):'—';
 if(typeof value==='object')return value.name??value.displayName??value.title??value.label??value.type??'جزئیات موجود';
 return String(value);
}
export function DataTable({columns,rows,empty='داده‌ای وجود ندارد.'}:{columns:{key:string;label:string}[];rows:Record<string,any>[];empty?:string}){
 return <div className="table-wrap"><table><thead><tr>{columns.map(c=><th key={c.key}>{c.label}</th>)}</tr></thead><tbody>{rows.length?rows.map((r,i)=><tr key={r.id??i}>{columns.map(c=><td key={c.key}>{tableValue(r[c.key])}</td>)}</tr>):<tr><td colSpan={columns.length}><Empty>{empty}</Empty></td></tr>}</tbody></table></div>
}
