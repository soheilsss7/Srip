...
export function OpsWorkspace({title,eyebrow,description,endpoint,actions=[]}:{title:string;eyebrow:string;description:string;endpoint:string;actions?:{label:string;path:string;method?:'POST'|'PATCH';body?:any}[]}){
 ...
 async function act(a:any){setBusy(a.label);setError('');try{await api(a.path,{method:a.method??'POST',body:a.body?JSON.stringify(a.body):undefined});await load()}catch(e){setError((e as Error).message)}finally{setBusy('')}}
