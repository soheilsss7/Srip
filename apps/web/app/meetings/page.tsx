...
export default function MeetingsPage() {
  ...
  const canRead = can('meeting.read');
  ...
  useEffect(() => { void load(); }, [upcoming, scopeId, canRead]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (Number.isNaN(startAt.getTime())) { setError('زمان شروع جلسه معتبر نیست.'); return; }
    if (endAt && (Number.isNaN(endAt.getTime()) || endAt <= startAt)) { setError('زمان پایان باید معتبر و بعد از شروع جلسه باشد.'); return; }
    setSaving(true); setError('');
    try {
      await api('/meetings', { method: 'POST', body: JSON.stringify({ ...form, startAt: startAt.toISOString(), endAt: endAt?.toISOString(), organizationId: form.organizationId || undefined, relationshipId: form.relationshipId || undefined, participantPersonIds: participantIds }) });
