...
export default function Calendar(){
 ...
 const load=async()=>{if(!canRead){setRows([]);setLoading(false);return}setLoading(true);setE('');try{const x=await api<any>(`/meetings?upcoming=${upcoming}&page=1&pageSize=100`);setRows(Array.isArray(x)?x:x?.items??x?.rows??[])}catch(x){setE((x as Error).message)}finally{setLoading(false)}};useEffect(()=>{void load()},[upcoming,canRead]);
 if(!canRead)return <main className="feature-page"><PageHeader eyebrow="CALENDAR / MEETINGS" title="تقویم جلسات" description="نمای عملیاتی جلسات."/><section className="panel"><p className="empty-state">مجوز مشاهده جلسات برای شما فعال نیست.</p></section></main>;
 return <main className="feature-page"><PageHeader eyebrow="CALENDAR / MEETINGS" title="تقویم جلسات" description="نمای عملیاتی جلسات با Participant، Organization، Relationship، Agenda و Outcome." actions={canWrite&&<button className="primary-action" onClick={()=>location.href='/meetings'}>+ جلسه جدید</button>}/><div className="toolbar"><button onClick={()=>setUpcoming(true)} className={upcoming?'primary-action':'secondary-action'}>پیش‌رو</button><button onClick={()=>setUpcoming(false)} className={!upcoming?'primary-action':'secondary-action'}>همه</button><button onClick={load} className="secondary-action">بازخوانی</button></div><ErrorCard message={e}/>{loading?<Loading/>:<section className="panel"><DataTable columns={['title','startAt','endAt','status','organization','relationship'].map(k=>({key:k,label:k}))} rows={rows.map(r=>({...r,organization:r.organization?.name??r.organization??'—',relationship:r.relationship?.name??r.relationship?.title??(r.relationship?'رابطه مرتبط':'—')}))}/></section>}</main>
}
