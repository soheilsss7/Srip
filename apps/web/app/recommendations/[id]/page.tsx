...
export default function Page({params}:{params:Promise<{id:string}>}){
 ...
 const load=useCallback(async()=>{if(!canRead){setR(null);return}setError('');try{const v:any=await api(`/recommendations/${id}`);setR(v);setTitle(v.title??'');setRationale(v.rationale??'');setConfidence(v.confidence!=null?String(v.confidence):'')}catch(e){setError((e as Error).message)}},[id,canRead]);
 ...
 async function showExplain(){if(!canRead)return;setExplanation(null);setError('');try{setExplanation(await api(`/recommendations/${id}/explain`))}catch(e){setError((e as Error).message)}}

 if(!canRead)return <main className="feature-page"><PageHeader eyebrow="RECOMMENDATION" title="Recommendation" description="جزئیات پیشنهاد و وضعیت اجرای آن."/><section className="panel"><p className="empty-state">مجوز مشاهده این پیشنهاد برای شما فعال نیست.</p></section></main>;
 return <main className="feature-page">
  ...
  {!r&&!error?<Loading/>:r&&<>
   ...
   {canWrite&&<section className="panel"><div className="panel-title"><h2>تصمیم</h2></div>
   ...
   {canWrite&&<section className="panel"><div className="panel-title"><h2>Snooze / Assign</h2></div>
   ...
   {canWrite&&<section className="panel"><div className="panel-title"><h2>ویرایش</h2></div>
