...
export default function Security(){
 ...
 const eventCols=[['type','نوع'],['severity','شدت'],['ipAddress','IP'],['entity','رکورد مرتبط'],['createdAt','زمان']].map(([key,label])=>({key,label}));
 const exportCols=[['exportType','نوع خروجی'],['entityType','نوع رکورد'],['recordCount','تعداد'],['classification','طبقه‌بندی'],['createdAt','زمان']].map(([key,label])=>({key,label}));
 ...
 {loading?<Loading/>:<>
  {canEvents&&<section className="panel"><div className="panel-title"><div><h2>Security Events</h2><p>{events.length} رویداد</p></div></div>{events.length===0?<Empty>رویدادی در Scope شما ثبت نشده است.</Empty>:<DataTable columns={eventCols} rows={events.map(r=>({...r,entity:r.entityType?'رکورد مرتبط':'—',severity:<Badge tone={sevTone(r.severity)}>{r.severity}</Badge>,createdAt:new Date(r.createdAt).toLocaleString()}))}/>}</section>}
