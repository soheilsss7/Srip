...
export default function Documents(){
 ...
 const canRead=can('document.read');
 ...
 const load=useCallback(async()=>{if(!canRead){setDocs([]);setStatus(null);setLoading(false);return}setLoading(true);setError('');try{const effectiveOrgId=orgId|| (scopeId!=='all'?scopeId:'');const params=effectiveOrgId?`?organizationId=${encodeURIComponent(effectiveOrgId)}`:'';
   const [docsR,statusR]=await Promise.all([api(`/documents${params}`),api('/documents/status')]);setDocs(unwrap(docsR));setStatus(statusR);}catch(x){setError((x as Error).message)}finally{setLoading(false)}},[orgId,scopeId,canRead]);
 useEffect(()=>{void load()},[load]);
 async function doUpload(){if(!canWrite)return;if(!file){setNotice('یک فایل انتخاب کنید.');return}setBusy('upload');setNotice('');setError('');try{const extra:Record<string,string>={classification};if(orgId)extra.organizationId=orgId;await apiUpload('/documents/upload',file,'file',extra);setFile(null);setNotice('فایل بارگذاری و اسکن شد.');await load()}catch(x){setError((x as Error).message)}finally{setBusy('')}}
 async function download(id:string){if(!canRead)return;setBusy('dl'+id);setError('');try{const r:any=await api(`/documents/${id}/signed-url`);if(r?.url)window.open(r.url,'_blank','noopener');else setError('Signed URL در دسترس نیست.');}catch(x){setError((x as Error).message)}finally{setBusy('')}}
 async function indexDoc(id:string){if(!canWrite)return;if(!indexText.trim()){setNotice('متن محتوا را برای Index وارد کنید.');return}setBusy('idx'+id);setNotice('');setError('');try{await api(`/documents/${id}/index`,{method:'POST',body:JSON.stringify({text:indexText})});setIdxFor('');setIndexText('');setNotice('محتوا ایندکس شد.');}catch(x){setError((x as Error).message)}finally{setBusy('')}}
 if(!canRead)return <main className="feature-page"><PageHeader eyebrow="DOCUMENTS" title="مدیریت اسناد" description="بارگذاری امن، اسکن بدافزار، ایندکس محتوا و دانلود امضاء‌شده با مجوز سازمانی."/><section className="panel"><Empty>مجوز مشاهده اسناد برای شما فعال نیست.</Empty></section></main>;
 return <main className="feature-page"><PageHeader eyebrow="DOCUMENTS" title="مدیریت اسناد" description="بارگذاری امن، اسکن بدافزار، ایندکس محتوا و دانلود امضاء‌شده با مجوز سازمانی." actions={canWrite&&<button className="primary-action" onClick={()=>document.getElementById('file')?.click()} disabled={!!busy}>بارگذاری سند</button>}/>
 ...
 {loading?<Loading/>:docs.length===0?<Empty>سندی در Scope شما نیست.</Empty>:<div className="list">{docs.map(d=><article className="panel compact" key={d.id}><div className="panel-title"><div><strong>{d.name}</strong><small className="muted">{d.mimeType} · {fmtBytes(d.sizeBytes)} · {d.classification} · {fmtDate(d.createdAt)}</small></div><span><Badge tone={toneFor(d.scanStatus)}>scan:{d.scanStatus}</Badge><Badge tone={toneFor(d.uploadStatus)}>up:{d.uploadStatus}</Badge></span></div><div className="toolbar">
  ...
  {canWrite&&<button className="secondary-action" onClick={()=>setIdxFor(idxFor===d.id?'':d.id)} disabled={!!busy}>{idxFor===d.id?'انصراف':'ایندکس'}</button>}
 </div>
