...
export default function Documents(){
 ...
 <section className="dashboard-grid">
  ...
  <article className="panel"><div className="panel-title"><div><h2>نقشه بارگذاری</h2><p>اعتبارسنجی MIME/Ext، قرنطینه، اسکن بدافزار و دسترسی امضاء‌شده.</p></div></div><input id="file" type="file" onChange={e=>setFile(e.target.files?.[0]??null)} disabled={!!busy}/>{file&&<div className="entity-form"><label>طبقه‌بندی<select value={classification} onChange={e=>setClassification(e.target.value)}>{['INTERNAL','CONFIDENTIAL','RESTRICTED','PUBLIC'].map(c=><option key={c} value={c}>{c}</option>)}</select></label><label>Organization ID (اختیاری)<input value={orgId} onChange={e=>setOrgId(e.target.value)} placeholder="uuid"/></label><button className="primary-action" onClick={doUpload} disabled={!!busy}>{busy==='upload'?'در حال اسکن…':'بارگذاری'}</button></div>}</article>
 </section>
 <section className="panel"><div className="panel-title"><div><h2>اسناد</h2><p>{docs.length} سند</p></div><div className="toolbar"><input value={orgId} onChange={e=>setOrgId(e.target.value)} placeholder="فیلتر Organization ID…" style={{maxWidth:220}}/><button className="secondary-action" onClick={load} disabled={!!busy}>اعمال فیلتر</button></div></div>
